...
def blue_white_veil(image_path, mask_path):
    ...
    cropped_image = cv2.bitwise_and(image, image, mask=mask) # crop the image using the mask so that we only work with the lesion
    hsv = cv2.cvtColor(cropped_image, cv2.COLOR_BGR2HSV) # convert the cropped image to HSV 
