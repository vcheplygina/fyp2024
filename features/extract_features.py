...
def asymmetry(mask_path):
    ...
    try:
        ...
        total_score = round(
            (vertical_symmetry_score + horizontal_symmetry_score) / 2, 4
        )

        return total_score
    except Exception as e:
        print(f"err: asymmetry, mask: {mask_path}")
        return None
...
def atypical_network(image_path, mask_path):
    ...
    try:
        ...
        kernel = np.ones(
            (3, 3), np.uint8
        )  # dilate edges to make the network more visible
        ...
        print(f"err: atypical_network, img {image_path}")
        return None
